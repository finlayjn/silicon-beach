...
if ( ! function_exists( 'silicon_beach_posted_on' ) ) :
	/**
...
if ( ! function_exists( 'silicon_beach_posted_by' ) ) :
	/**
	 * Prints HTML with meta information about theme author.
	 */
	function silicon_beach_posted_by() {
		printf(
		/* translators: 1: posted by label, only visible to screen readers. 2: author link. 3: post author. */
			'<span class="sr-only">%1$s</span><span class="author vcard"><a class="url fn n" href="%2$s">%3$s</a></span>',
			esc_html__( 'Posted by', 'silicon-beach' ),
...
if ( ! function_exists( 'silicon_beach_comment_count' ) ) :
	/**
	 * Prints HTML with the comment count for the current post.
	 */
	function silicon_beach_comment_count() {
		if ( ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
			/* translators: %s: Name of current post. Only visible to screen readers. */
			comments_popup_link( sprintf( __( 'Leave a comment<span class="sr-only"> on %s</span>', 'silicon-beach' ), get_the_title() ) );
		}
	}
endif;

if ( ! function_exists( 'silicon_beach_entry_meta' ) ) :
	/**
	 * Prints HTML with meta information for the categories, tags and comments.
	 * This template tag is used in the entry header.
	 */
	function silicon_beach_entry_meta() {
...
if ( ! function_exists( 'silicon_beach_entry_footer' ) ) :
	/**
	 * Prints HTML with meta information for the categories, tags and comments.
	 */
	function silicon_beach_entry_footer() {
...
if ( ! function_exists( 'silicon_beach_post_thumbnail' ) ) :
	/**
	 * Displays an optional post thumbnail, wrapping the post thumbnail in an
	 * anchor element except when viewing a single post.
	 */
	function silicon_beach_post_thumbnail() {
		if ( ! silicon_beach_can_show_post_thumbnail() ) {
			return;
		}
...
if ( ! function_exists( 'silicon_beach_comment_avatar' ) ) :
	/**
	 ...
	 */
	function silicon_beach_get_user_avatar_markup( $id_or_email = null ) {

		if ( ! isset( $id_or_email ) ) {
			$id_or_email = get_current_user_id();
		}

		return sprintf( '<div class="vcard">%s</div>', get_avatar( $id_or_email, silicon_beach_get_avatar_size() ) );
	}
endif;

if ( ! function_exists( 'silicon_beach_discussion_avatars_list' ) ) :
	/**
	 * Displays a list of avatars involved in a discussion for a given post.
	 *
	 * @param array $comment_authors Comment authors to list as avatars.
	 */
	function silicon_beach_discussion_avatars_list( $comment_authors ) {
		...
		foreach ( $comment_authors as $id_or_email ) {
			printf(
				"<li>%s</li>\n",
				silicon_beach_get_user_avatar_markup( $id_or_email ) // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			);
		}
		echo '</ol>', "\n";
	}
...
if ( ! function_exists( 'silicon_beach_content_class' ) ) :
	/**
	 ...
	 * SILICON_BEACH_TYPOGRAPHY_CLASSES constant.)
	 *
	 ...
	 */
	function silicon_beach_content_class( $classes = '' ) {
		$all_classes = array( $classes, SILICON_BEACH_TYPOGRAPHY_CLASSES );
